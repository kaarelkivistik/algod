...
public class Eve {
	...
	//
	private SecretKey aliceSecret, bobSecret;
	...
	public void interceptKey(Actor recipient, Key key) {
		try {
            SecretKey secretKey = (SecretKey) DH.generateAESKey(privateKey, (PublicKey) key);

            if(recipient instanceof Bob) {
				System.out.println("Eve: [Alice --[key]--> Bob]: " + DH.exportKey(key));

				aliceSecret = secretKey;
			} else if(recipient instanceof Alice) {
				System.out.println("Eve: [Bob --[key]--> Alice]: " + DH.exportKey(key));
	...
	public void interceptMessage(Actor recipient, String msg) {
		String recryptedMessage = null;

		//TODO: Edit this function
		if(recipient instanceof Bob) {
			System.out.println("Eve: [Alice --[msg]--> Bob]: " + msg);

			try {
				recryptedMessage = DH.encryptAES(bobSecret, DH.decryptAES(aliceSecret, msg));
			...
			try {
				recryptedMessage = DH.encryptAES(aliceSecret, DH.decryptAES(bobSecret, msg));
			} catch (AESCipherException e) {
				e.printStackTrace();
			}
		}

		CommunicationChannel.sendInterceptedMsgTo(recipient, recryptedMessage);
	}
